...
export const getDeploymentStatusTool = (authToken?: string) =>
  tool({
    ...
    execute: async (params) => {
      try {
        ...
        return {
          ...
          url: result.app_url || result.app_url,
          resources: {
...
export const listDeploymentsTool = (authToken?: string) =>
  tool({
    description: "List all deployments for the current user",
    parameters: z.object({
      status: z
        .enum(["all", "running", "stopped", "pending"])
        .default("all")
        .describe("Filter by deployment status"),
      serviceType: z
        .enum(["all", "BACKEND", "FRONTEND", "JUPYTER", "VMS", "MODELS"])
        .default("all")
      ...
      try {
        ...
        if (params.status !== "all") {
          deployments = deployments.filter(
            (dep: any) =>
              dep.status?.toLowerCase() === params.status.toLowerCase()
          );
        }
        ...
        const transformedDeployments = deployments.map((dep) => ({
          ...
          status: isDeploymentActive(dep.created_at, dep.duration)
            ? "running"
            : "stopped",
          url: dep.app_url || "",
...
export const getResourcePricingTool = () =>
  tool({
    description:
      "Get pricing information for different resource configurations",
    parameters: z.object({
      provider: z
