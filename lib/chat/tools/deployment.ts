...
export const deployApplicationTool = (authToken?: string) =>
  tool({
    description:
      "Deploy an application from a Git repository to decentralized compute infrastructure",
    parameters: z.object({
      ...
      serviceType: z
        ...
        .default("BACKEND")
        .describe("Type of application service"),
      provider: z
        .enum(["akash", "spheron", "auto"])
        .default("auto")
      ...
      cpuUnits: z
        ...
        .max(4)
        .default(1)
        .describe("CPU units (0.5-4)"),
      memorySize: z
        .string()
        .default("1Gi")
        .describe("Memory allocation (e.g., 1Gi, 2Gi)"),
      storageSize: z
        .string()
        .default("1Gi")
        .describe("Storage allocation (e.g., 1Gi, 5Gi)"),
      port: z.number().default(3000).describe("Application port"),
      duration: z
        .string()
        .default("24h")
        .describe("Deployment duration (e.g., 1h, 24h, 7d)"),
      envVars: z
        .record(z.string())
        .optional()
    ...
    execute: async (params) => {
      try {
        ...
        return {
          status: "pending",
          deploymentId: result.id || `dep_${Date.now()}`,
...
export const deployJupyterTool = (authToken?: string) =>
  tool({
    description:
      "Deploy a Jupyter notebook environment for data science and machine learning",
    parameters: z.object({
      provider: z
        .enum(["akash", "spheron", "auto"])
        .default("auto")
      ...
      pythonPackages: z
        .array(z.string())
        .optional()
        .describe("Additional Python packages to install"),
    }),
    execute: async (params) => {
      try {
        const config = {
        ...
        return {
          status: "pending",
          deploymentId: result.id || `jupyter_${Date.now()}`,
          message: "Jupyter notebook environment deployment initiated",
          config: params,
          accessUrl: result.accessUrl || result.app_url,
...
export const deployVMTool = (authToken?: string) =>
  tool({
    description: "Deploy a virtual machine with custom configuration",
    parameters: z.object({
      vmType: z
        .enum(vmsConfig.map((vm) => vm.slug) as [string, ...string[]])
        .default("ubuntu-24-vm")
        .describe("VM type/image to deploy"),
      ...
      storageSize: z
        .string()
        .default("10Gi")
        .describe("Storage allocation (e.g., 1Gi, 20Gi)"),
      duration: z
        .string()
        .default("24h")
        .describe("Deployment duration (e.g., 1h, 24h, 7d)"),
    ...
    execute: async (params) => {
      try {
        ...
        return {
          status: "pending",
          deploymentId: result.id || `vm_${Date.now()}`,
...
export const deployInferenceTool = (authToken?: string) =>
  tool({
    description: "Deploy AI/ML model inference endpoints",
    parameters: z.object({
      modelType: z
        .enum(["llm", "vision", "audio", "custom"])
        .describe("Type of AI model"),
      modelName: z
        .string()
        .describe(
          'Model name or identifier (e.g., "gpt-3.5-turbo", "llama-2-7b")'
        ),
      provider: z
        .enum(["akash", "spheron", "auto"])
        .default("auto")
      ...
      cpuUnits: z
        ...
        .max(4)
        .default(4)
        .describe("CPU units (2-4, inference needs more compute)"),
      memorySize: z
        .string()
        .default("4Gi")
        .describe("Memory allocation (e.g., 2Gi, 8Gi)"),
      storageSize: z
        .string()
        .default("10Gi")
        .describe("Storage allocation for model weights"),
      duration: z
        .string()
        .default("24h")
        .describe("Deployment duration (e.g., 1h, 24h, 7d)"),
      gpuEnabled: z
        .boolean()
        .default(false)
        .describe("Enable GPU acceleration"),
      gpuModel: z
        .string()
        .optional()
        .describe('GPU model (e.g., "rtx4090", "a100")'),
    }),
    execute: async (params) => {
      try {
        ...
        return {
          status: "pending",
          deploymentId: result.id || `inference_${Date.now()}`,
